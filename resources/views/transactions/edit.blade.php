<x-app-layout>
    <div class="mb-6 mx-auto" style="max-width: 600px;">
        ...
        <h2 class="title-gradient page-title">Edit Transaction</h2>
    </div>
        ...
        <form action="{{ route('transactions.update', $transaction) }}" method="POST" class="space-y-4">
            ...
            <div class="form-group" style="margin-bottom: 0.75rem;">
                ...
                <div class="flex gap-4" style="margin-top: 0.5rem;">
                    <label class="flex items-center gap-2" style="cursor: pointer;">
                        <input type="radio" name="type" value="expense" {{ old('type', $transaction->type) === 'expense' ? 'checked' : '' }} onchange="filterCategories()">
                        <span>Expense</span>
                    </label>
                    <label class="flex items-center gap-2" style="cursor: pointer;">
                        <input type="radio" name="type" value="income" {{ old('type', $transaction->type) === 'income' ? 'checked' : '' }} onchange="filterCategories()">
                        <span>Income</span>
                    </label>
                </div>
                @error('type')
            ...
            <div class="dashboard-grid" style="grid-template-columns: 1fr 1fr; gap: 0.9rem; margin-bottom: 0;">
                ...
                <div class="form-group" style="margin-bottom: 0.75rem;">
                    ...
                    <input type="number" step="0.01" min="0.01" name="amount" id="amount" class="form-control" value="{{ old('amount', $transaction->amount) }}" required>
                    @error('amount')
                        <p style="color: var(--danger); font-size: 0.85rem; margin-top: 0.5rem;">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="dashboard-grid" style="grid-template-columns: 1fr 1fr; gap: 0.9rem; margin-bottom: 0;">
                ...
                <div class="form-group" style="margin-bottom: 0.75rem;">
                    ...
                    <input type="text" name="payment_method" id="payment_method" class="form-control" placeholder="e.g. Credit Card, Cash" value="{{ old('payment_method', $transaction->payment_method) }}">
            ...
            <div class="form-group" style="margin-bottom: 0.75rem;">
                ...
                <input type="text" name="description" id="description" class="form-control" placeholder="What was this for?" value="{{ old('description', $transaction->description) }}" required>
                @error('description')
                    <p style="color: var(--danger); font-size: 0.85rem; margin-top: 0.5rem;">{{ $message }}</p>
                @enderror
            </div>
